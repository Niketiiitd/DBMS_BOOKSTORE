...
def approve_or_block_vendors(pending_vendors, vendor_msgs):
    ...
    for vendor in pending_vendors:
        if vendor["email"] == email:
            ...
            if choice == "1":
                approve_vendor_registration(email )
                approved = True
                break
            elif choice == "2":
...
def block_vendor(v_id):
    try:
        # Execute the SQL update statement
        cursor.execute("UPDATE Vendor SET vendor_banned = 1 WHERE vendor_id = %s", (v_id,))
        # Commit the transaction
        mydb.commit()
        print(f"Vendor with ID {v_id} has been successfully blocked.")
    except Exception as e:
        # Rollback the transaction if an error occurs
        print("Error:", e)
        mydb.rollback()
def Unblock_vendor(v_id):
    try:
        # Execute the SQL update statement
        cursor.execute("UPDATE Vendor SET vendor_banned = 0 WHERE vendor_id = %s", (v_id,))
        # Commit the transaction
        mydb.commit()
        print(f"Vendor with ID {v_id} has been successfully Unblocked.")
    except Exception as e:
        # Rollback the transaction if an error occurs
        print("Error:", e)
        mydb.rollback()
...
def manage_warehouse_details():
    while True:
        ...
        if choice == '1':
            ...
            warehouse_id = input("Enter the Warehouse id which you want to remove:")
            remove_warehouse(warehouse_id)
        elif choice == '3':
            show_all_warehouses()
        elif choice == '4':
